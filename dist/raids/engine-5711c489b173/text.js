...
class Match extends Array {
    ...
    group(name = 0) { return typeof name === 'number' ? this.result[name] : this.result.groups?.[name]; }
}
function regex(pattern, flags = '', full = false) {
    const source = pattern.replace(/\(\?P</g, '(?<');
    return new RegExp(full ? '^(?:' + source + ')$' : source, flags);
}
export const re = {
    I: 'i',
    compile(pattern, flags = '') { return { fullmatch(text) { const m = regex(pattern, flags, true).exec(text); return m ? new Match(m) : null; } }; },
    fullmatch(pattern, text, flags = '') { return this.compile(pattern, flags).fullmatch(text); },
    findall(pattern, text) { return Array.from(text.matchAll(regex(pattern, 'g')), m => m[0]); },
};
export function seconds_to_tick(value, line_number) {
    const fail = (message) => { throw new Error(`Line ${line_number}: ${message}`); };
...
export function literal(text) {
    ...
    function value(depth = 0) {
        ...
        for (const [word, result] of [['True', true], ['False', false], ['None', null], ['true', true], ['false', false], ['null', null]]) {
