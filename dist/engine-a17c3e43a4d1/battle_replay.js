        ...
        if (py.truth(((py.equal(this.action_kind, "boss_fast"))))) {
            ...
            if (py.truth(((py.equal(this.action_kind, "boss_charged"))))) {
                ...
                if (py.truth(((py.equal(this.action_kind, "move"))))) {
                    ...
                    else if (py.truth(((py.equal(this.action_kind, "gem"))))) {
...
function parse_event(line, line_number, previous_tick) {
    ...
    if (py.truth(((py.equal(action, "g"))))) {
        return new ReplayEvent(tick, players, "gem", action, line_number);
    }
...
function parse_replay_text(text) {
    ...
    boss = py.dict([]);
    players = py.dict([]);
    settings = py.dict([["weather", null], ["dodge_strategy", null], ["player_strategy", null], ["use_purified_gems", false], ["party_power", py.dict([["mode", "normal"], ["groups", []]])]]);
    warnings = [];
    move_codes = py.dict([]);
    events = [];
    in_events = false;
