...
self.onmessage = async (event) => {
    const { id, mode, bossFormId, raidDifficulty, includeMegas, includeShadows, includeLegendaries, megaLevel, level, friendshipMultiplier, weather, dodgeStrategy, playerStrategy, excludeLegacy, trialsPerBossMoveset, prefilterLimit, } = event.data ?? {};
    try {
        ...
        const result = calculateRaidCounters(catalog, {
            bossFormId,
            raidDifficulty,
            ...
            prefilterLimit,
        }, shadowFormIds, legendaryDexNumbers, (completed, total) => self.postMessage({ id, progress: { completed, total } }));
