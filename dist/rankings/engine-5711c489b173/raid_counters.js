...
export function calculateRaidCounters(catalog, settings, shadowFormIds = new Set(), legendaryDexNumbers = new Set(), onProgress) {
    ...
    for (let candidateIndex = 0; candidateIndex < selected.length; candidateIndex += 1) {
        ...
        const makeMove = (move) => new engine.Move(move.name, move.power, move.duration_ms / 1000, Math.abs(move.energy), titleType(move.type), false, move.plus_powers?.length === 4
            ? move.plus_powers
            : null);
        ...
        for (let pairIndex = 0; pairIndex < bossMovePairs.length; pairIndex += 1) {
            ...
            for (let trial = 0; trial < trialsPerBossMoveset; trial += 1) {
                ...
                const dodgeProfiles = Object.create(null);
                team.forEach((pokemon, pokemonIndex) => {
                    const effectiveness = engine.type_effectiveness(pair.engineCharged.move_type, pokemon.species.types);
                    for (const enraged of [false, true]) {
                        const fullDamage = engine.incoming_damage_for_pokemon(pair.engineCharged, pokemon, enraged, false, 1);
                        const dodgedDamage = engine.incoming_damage_for_pokemon(pair.engineCharged, pokemon, enraged, true, 1);
                        dodgeProfiles[JSON.stringify([0, pokemonIndex, enraged])] = new engine.DodgeProfile(fullDamage, dodgedDamage, effectiveness > 1, effectiveness < 1, false);
                    }
                });
                const simulation = new engine.Simulation(pair.engineFast, pair.engineCharged, new PythonRandom(seed), dodgeProfiles);
