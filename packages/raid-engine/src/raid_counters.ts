...
export interface RaidCounterSettings {
    ...
    /** Empty/omitted means average all eligible moves of this kind. */
    bossFastMoveId?: string;
    bossChargedMoveId?: string;
...
export interface RaidCounterRow {
    ...
    movesetResults: {fastMoveId: string; chargedMoveId: string; battleDps: number; averageFaints: number}[];
}

export interface BossMoveOption { id: string; name: string; type: string }
export interface BossMovesetDifficulty {
    fastMoveId: string;
    chargedMoveId: string;
    fastMove: string;
    chargedMove: string;
    averageBattleDps: number;
    averageFaints: number;
    /** Easiest tested moveset = 100. Higher means harder for this fixed lineup. */
...
export interface RaidCounterResult {
    ...
    movesetDifficulty: BossMovesetDifficulty[];
    rows: RaidCounterRow[];
    assumptions: {
        level: RaidCounterLevel;
        attackIV: 15;
...
export interface RaidBossOption {
    ...
    fastMoves: BossMoveOption[];
    chargedMoves: BossMoveOption[];
}
...
export function counterSimulation(
    ...
    const species = new engine.Species(candidate.entry.name, candidate.entry.stats.attack,
        ...
        candidate.entry.types.map(titleType), false, [], false, candidate.entry.form_id);
    const makeMove = (move: CalculatorMove) => new engine.Move(move.name, move.power,
        move.duration_ms / 1000, Math.abs(move.energy), titleType(move.type), false,
        move.plus_powers?.length === 4 ? move.plus_powers as [number, number, number, number] : null);
    ...
    const profiles: Record<string, InstanceType<typeof engine.DodgeProfile>> = Object.create(null);
    team.forEach((pokemon, index) => {
        ...
        for (const enraged of [false, true]) {
            profiles[JSON.stringify([0, index, enraged])] = new engine.DodgeProfile(
                engine.incoming_damage_for_pokemon(pair.engineCharged, pokemon, enraged, false, 1),
                engine.incoming_damage_for_pokemon(pair.engineCharged, pokemon, enraged, true, 1),
                effectiveness > 1, effectiveness < 1, false);
        }
    });
...
export function counterCandidate(
    catalog: CalculatorEntry[], pick: {formId: string; fastMoveId: string; chargedMoveId: string; shadow: boolean},
 ...
 */
export function prepareRaidCounterScenario(
    ...
    const filterBossMoves = (moves: CalculatorMove[], id: string | undefined) => {
        if (!id) return moves;
        const move = moves.find(move => move.id === id);
        if (!move) throw new Error('Choose an ordinary moveset belonging to this boss.');
        return [move];
    };
    ...
    const allBossMovePairs: BossMovePair[] = bossFastMoves.flatMap(fastMove => bossChargedMoves.map(chargedMove => ({
...
export function calculateRaidCounters(
    catalog: CalculatorEntry[], settings: RaidCounterSettings,
    ...
    for (let candidateIndex = 0; candidateIndex < selected.length; candidateIndex += 1) {
        ...
        let winSeconds = 0;
        let battles = 0;
        const movesetResults: RaidCounterRow['movesetResults'] = [];
    ...
    const movesetDifficulty: BossMovesetDifficulty[] = bossMovePairs.map((pair, index) => ({
