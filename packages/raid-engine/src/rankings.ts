...
export const BOSS_MOVE_TYPES = ['typeless', ...RANKING_TYPES] as const;
export type BossMoveType = typeof BOSS_MOVE_TYPES[number];

export const BOSS_ATTACK_LEVELS = ['low', 'medium', 'high'] as const;
export type BossAttackLevel = typeof BOSS_ATTACK_LEVELS[number];
...
export interface RankingSettings {
    ...
    bossAttack?: BossAttackLevel;
    bossMoveType?: BossMoveType;
    megaLevel?: MegaLevel;
}
...
export interface RankingResult {
    ...
    megaRows: number;
    shadowRows: number;
    legendaryRows: number;
    rows: RankingRow[];
    assumptions: {
        attackIV: 15;
        defenseIV: 15;
        staminaIV: 15;
        pulseSecondsMin: number;
        pulseSecondsMax: number;
        ...
        shadowIncomingMultiplier: number;
        megaPlusPowerMultiplier: number;
        superMaxLevelBoost: number;
    };
}
...
interface ExpectedAppearance extends Appearance {
    firstChargedProbability: number;
}
...
const TARGET_DEFENSES = [160, 180, 200, 220, 240] as const;
...
const BOSS_ATTACK_COEFFICIENTS: Readonly<Record<BossAttackLevel, number>> = {
    low: 675,
    medium: 900,
    high: 1125,
};
...
const TYPE_RELATIONSHIPS: Readonly<Record<RankingType, {
    superEffective: ReadonlySet<string>;
...
function bossMoveEffectiveness(moveType: BossMoveType, defenderTypes: string[]): number {
    ...
    return defenderTypes.reduce((multiplier, defenderType) => {
        ...
        if (relationships.immune.has(normalized)) return multiplier * 0.390625;
        if (relationships.superEffective.has(normalized)) return multiplier * 1.6;
        if (relationships.resisted.has(normalized)) return multiplier * 0.625;
        return multiplier;
    }, 1);
}
...
function moveDamage(
    ...
    return Math.floor(
        ...
        * stab * effectiveness * shadowMultiplier,
    ) + 1;
}
...
function unitRandom(seed: number, index: number, stream: number): number {
...
function pulseInterval(seed: number, index: number): number {
...
function canReachNextCharged(
    ...
    for (let actions = 0; actions < 1000 && hp > 0; actions += 1) {
        ...
        if (selected === charged) energy -= charged.energy;
        ...
        time = actionEnd;
        if (selected === charged) return true;
        energy = Math.min(100, energy + fast.energy);
...
function nextChargedProbability(
    stats: EffectiveStats,
    hp: number,
    energy: number,
    fast: MoveDamage,
    ...
    for (let sample = 0; sample < CONTINUATION_SAMPLES; sample += 1) {
        ...
        if (canReachNextCharged(
            stats, hp, energy, fast, charged, shadow, bossAttackCoefficient, seed,
        )) successes += 1;
    }
...
function appearanceScenario(
    ...
    for (let actions = 0; actions < 10000 && hp > 0; actions += 1) {
        ...
        if (selected === charged) {
            const probability = nextChargedProbability(
                stats,
                hp,
                energy,
                fast,
...
function expectedAppearance(
    ...
    for (let scenario = 0; scenario < PULSE_SCHEDULE_COUNT; scenario += 1) {
        const appearance = appearanceScenario(
            stats,
            fast,
            charged,
            shadow,
...
function isBetter(left: RankingRow, right: RankingRow): boolean {
...
export function calculateRankings(
    ...
    if (!BOSS_MOVE_TYPES.includes(bossMoveType)) throw new Error(`Unknown boss move type: ${settings.bossMoveType}`);
    const megaLevel = settings.megaLevel ?? 1;
    if (!MEGA_LEVELS.includes(megaLevel)) throw new Error(`Unknown Mega Level: ${settings.megaLevel}`);
    ...
    const bestByForm = new Map<string, RankingRow>();
        ...
        for (const shadow of shadowStates) {
            for (const candidateFast of playerFastMoves(entry).filter(move => move.energy > 0)) {
                ...
                for (const chargedMove of chargedMoves) {
                    ...
                    if (!previous || isBetter(row, previous)) bestByForm.set(key, row);
                }
            }
        }
    }

    const rows = [...bestByForm.values()].sort((a, b) =>
        b.idealDps - a.idealDps
