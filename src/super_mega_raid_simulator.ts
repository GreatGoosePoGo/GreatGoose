...
export function createRaidEngine(input: RaidConfig, catalog: CalculatorEntry[]) {
    ...
    ENRAGE_HP = (py.truth(SUPER_MEGA_ENRAGE) ? Math.floor(py.mul(BOSS_HP, 0.8)) : -1);
    FAST_MOVE_DELAY = 2.5;
    ...
    function incoming_damage_for_pokemon(move: Move, pokemon: BattlePokemon, enraged: boolean, dodged: boolean, defense_multiplier: number = 1.0): number {
        ...
        if (py.truth(enraged)) {
            modifier = py.mul(modifier, ENRAGE_ATTACK_MULTIPLIER);
        }
        if (py.truth(pokemon.is_shadow)) {
            modifier = py.mul(modifier, 1.2);
        }
        damage = pokemon_go_damage(move.power, py.mul(BOSS_ATTACK, BOSS_CPM), py.mul(pokemon.effective_defense, defense_multiplier), modifier);
    ...
    function precompute_dodge_profiles(boss_charged: Move): Record<string, DodgeProfile> {
        ...
        profiles = py.dict([]);
        enrage_states = (py.truth(SUPER_MEGA_ENRAGE) ? [false, true] : [false]);
    ...
    class TrialResult {
        constructor(public won: boolean, public finish_time: number, public boss_hp: number, public switches: number, public faints: number, public tactical_switches: number, public rejoins: number, public catch_tanks_used: number) { }
    }
    class Simulation {
        constructor(public boss_fast: Move, public boss_charged: Move, public rng: PythonRandom, public dodge_profiles: Record<string, DodgeProfile> | null = null, public boss_hp: number = BOSS_HP, public boss_energy: number = 0.0, public enraged: boolean = false, public players: (Player)[] = [], public events: (any[])[] = [], public sequence: number = 0, public detailed: boolean = false, public event_log: (string)[] = [], public replay_actions: ([
            ...
            string,
            any
        ])[] = [], public replay_sequence: number = 0, public current_time: number = 0.0) { this.__post_init__(); }
        __post_init__(): void {
            let species: any;
            let fast: any;
            let charged: any;
        ...
        outgoing_damage(move: Move, player_id: number, party_power: boolean = false): number {
            ...
            defense = py.mul(py.mul(BOSS_DEFENSE, BOSS_CPM), (py.truth(this.enraged) ? 4 : 1));
